...
/**
 * Fill the location step (City + State)
 */
export async function fillLocation(page: Page, city: string = "Atlanta", state: string = "GA") {
 ...
 */
export async function fillCard(page: Page, cardNumber: string, expMonth: string, expYear: string, cvv: string = "123") {
...
export async function fillPersonalInfo(
  ...
  const hasTeamStep = await page
    .locator('input[name="name_team"]')
  ...
  // Location (City/State) step follows Team on flows that show it (CO-I240)
  const hasLocationStep = await page
    .locator('input[name="city"]')
    .isVisible({ timeout: 2000 })
    .catch(() => false)
  if (hasLocationStep) {
    await fillLocation(page, data.city, data.state)
  }
