...
export class RequestMethods {
  ...
  async getWhitelabel() {
    console.warn("req.getWhitelabel is not implemented")
    return {
      id: 1,
      logoText: "Logo Text",
    }
  ...
  validateSchema = <T extends z.ZodType<any, any>>(schema: T, data: any): z.infer<T> => {
    try {
      return schema.parse(data)
    } catch (error: ZodError | any) {
      if (!Array.isArray(error?.issues) || error.issues.length === 0) {
        throw new CodedError("Invalid request data", 400, "REQ|03")
      }

      const issueMessage = error.issues
        .map((issue: any) => {
          const baseMessage = `${issue?.code}: ${issue?.path?.[0]}`
          if (issue?.message) return `${baseMessage}. ${issue?.message}`
          if (issue?.expected) return `${baseMessage}. Expected ${issue?.expected} but received ${issue?.received}`

          return baseMessage
        })
        .join(" | ")

      throw new CodedError(issueMessage || "Invalid request data", 400, "REQ|04")
    }
  }
  ...
  insertErrorInstance(error: Error, status: number) {
    ...
    this.onFinish("log error instances", async () => {
      ...
      if (Number(todayErrorCount?.count_today) === 5) {
        await sendChatMessage(`⚠️ An Error has occurred multiple times today: 
...
class Router {
  ...
  route(method: Method, path: string, config: RouteConfigOptions, ...handlers: Handler[]) {
    const wrappedHandlers = handlers.map((handler) => async (req: Req, res: Res, next: NextFunction) => {
      ...
      req.getWhitelabel = requestMethods.getWhitelabel.bind(requestMethods)
      req.validateSchema = requestMethods.validateSchema.bind(requestMethods)
      ...
      if (process.env.NODE_ENV === "test") await db.destroy()
    })
